    ...
    def __init__(self, file_path):
        ...
        if isinstance(self.file_path, str):
            if self.file_path.endswith(".nev"):
                ...
                raise IOError(
                    "Expected '.nev' file as the input. Received a '"
                    + file_extension
                    + "' file."
                )
        else:
...
class RippleEvents:
    ...
    def metadata(self):
        ...
        if self.type == "stim":
            ...
            for i, entity in enumerate(
                self.ripple_io.nsfile.get_entities(EntityType.segment)
            ):
                ...
                dict_info = {
                    "info_" + k: (v.split("\0")[0] if isinstance(v, str) else v)
                    for k, v in info.items()
                }
                dict_segment = {
                    "segment_" + k: (v.split("\0")[0] if isinstance(v, str) else v)
                    for k, v in segment.items()
                }
                dict_source = {
                    "source_" + k: (v.split("\0")[0] if isinstance(v, str) else v)
            ...
            stim_bool = [
                True if count > 0 else False for count in metadata["info_item_count"]
            ]
            ...
            metadata["source_channels"] = [
                i for i, channel in enumerate(stim_bool, start=1) if channel
            ]
    ...
    def event_data(self, channel, index=None):
        ...
        if isinstance(channel, int) and channel in self.metadata["source_channels"]:
            channel = self.metadata["info_label"][
                self.metadata["source_channels"].index(channel)
            ]
        elif isinstance(channel, str) and channel in self.metadata["info_label"]:
            pass
        else:
            raise ValueError(
                "channel is either not of type 'str' or 'int' or was not found"
            )
        ...
        for entity in entities:
            ...
            if isinstance(channel, str) and entity_info["label"] == channel:
                if index is None:
                    return np.asarray(
                        [
                            entity.get_segment_data(idx)[0]
                            for idx in range(entity_info["item_count"])
                        ]
                    )
    ...
    def __getitem__(self, item):
        if isinstance(item, (str, int)):
            ...
            raise TypeError(
                "RippleEvent class can only be indexed using 'str' or 'int' types"
            )
...
class RippleArray:
    ...
    def __init__(self, ripple_io, type="ephys"):
        if isinstance(ripple_io, RippleIO):
            ...
            self.type = type
            self.shape = (
                len(self.metadata["info_label"]),
                self.metadata["info_item_count"],
            )
        else:
            raise TypeError("input expected to by of type RippleIO")
    ...
    def metadata(self):
        ...
        if (
            self.type == "ephys"
        ):  # TODO: Refactor this to eliminate duplication of concepts for loading metadata
            # between types
            # get the appropriate information for each channel
            for i, entity in enumerate(
                self.ripple_io.nsfile.get_entities(EntityType.analog)
            ):
                with self.ripple_io.lock:
                    info_raw = entity.get_entity_info()._asdict()
                ...
                dict_analog = {
                    "analog_" + k: (v.split("\0")[0] if isinstance(v, str) else v)
                    for k, v in analog.items()
                }
                dict_header = {
                    "header_" + k: (v.split("\0")[0] if isinstance(v, str) else v)
            ...
            if isinstance(items, tuple):
                if len(items) == 0:
                    ...
                    if isinstance(items[1], (list, int, np.ndarray)):
                        ...
                        if (
                            items[1].start is None
                            and items[1].stop is None
                            and items[1].step is None
                        ):
                            return self[items[0]]
                        else:
                            # TODO: handle negative start and stop indices
                            start_index = (
                                0 if items[1].start is None else items[1].start
                            )
                            stop_index = (
                                self.shape[1]
                                if items[1].stop is None
                                else items[1].stop
                            )
                    ...
                    if isinstance(items[0], (list, int, np.ndarray)):
                        item_list = (
                            ...
                            else np.array(items[0])
                        )
                        if issubclass(item_list.dtype.type, np.integer):
                            data_list = []
                            for i in item_list:
                                with self.ripple_io.lock:
                                    data = entities[i].get_analog_data(
                                        start_index,
                ...
                item_list = (
                    np.array([items]) if isinstance(items, int) else np.array(items)
                )
                if issubclass(item_list.dtype.type, np.integer):
                    data_list = []
                    for i in item_list:
                        with self.ripple_io.lock:
                            data_list.append(
                                entities[i].get_analog_data(
                                    0, self.shape[1], use_scale=True
                                )
                            )
                ...
                with self.ripple_io.lock:
                    data_list = [
                        entity.get_analog_data(0, self.shape[1], use_scale=True)
