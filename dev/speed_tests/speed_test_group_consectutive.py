...
def test_functions():
    ...
    test_cases = [
        ...
        (np.array([1, 2, 3, 5, 6, 7, 9]), [np.array([1, 2, 3]), np.array([5, 6, 7]), np.array([9])]),
        (np.array([1, 3, 5, 7]), [np.array([1]), np.array([3]), np.array([5]), np.array([7])]),
        (np.array([]), []),
    ]

    # List of functions to test
    functions = [_group_consecutive_numpy, _group_consecutive_numba, _group_consecutive_itertools, _group_consecutive_python]
    ...
    for func in functions:
        for input_array, expected_output in test_cases:
            ...
            try:
                assert all(np.array_equal(a, b) for a, b in zip(output, expected_output)), f"Function {func.__name__} failed for input {input_array}. Expected {expected_output}, got {output}."
                print(f"Test passed for function {func.__name__} with input {input_array}.")
...
def generate_test_data(size, max_segment_length=100, max_step=1):
    ...
    while len(data) < size:
        ...
        data.extend(segment[:max(0, size - len(data))])  # Ensure we don't exceed the desired size
...
array_sizes = [100000, 1000000, 10000000, 50000000, 100000000]  # Added larger sizes for numpy and numba versions
...
with tqdm(total=total_steps, desc="Benchmarking Functions") as pbar:
    for size in array_sizes:
        ...
        for func in [_group_consecutive_numpy, _group_consecutive_numba, _group_consecutive_itertools, _group_consecutive_python]:
            # Skip the larger sizes for itertools and python versions
            if size > 10000000 and func.__name__ in ['_group_consecutive_itertools', '_group_consecutive_python']:
                pbar.update(1)
                continue
